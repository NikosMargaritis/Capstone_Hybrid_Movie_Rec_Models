...
DNN_HIDDEN_LAYER_3 = 128  # Set 0 tuse o not this layer
...
data = pd.read_csv("processed_data/wide_and_deep_input_file.csv")
...
if len(RANKING_METRICS) > 0:
    ...
    model_predictions = np.array([prediction_df['prediction'].values.tolist()])
    scaled_preds = scaler.fit_transform(model_predictions.T)
    # prediction_df['scaled_predictions'] = scaled_preds.T[0]
    to_save_predictions_df = prediction_df[['userID', 'itemID', 'genre']]
    to_save_predictions_df['scaled_predictions'] = scaled_preds.T[0]
    to_save_predictions_df.to_csv("processed_data/wide_and_deep_predictions.csv", index=False)
